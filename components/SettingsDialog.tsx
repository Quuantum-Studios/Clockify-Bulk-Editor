...
export default function SettingsDialog({ open, onClose, canClose = true }: SettingsDialogProps) {
  ...
  const loadSettings = useCallback(async () => {
    try {
      const res = await fetch(`/api/kv/settings?apiKey=${apiKey}`)
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      if (!isValidated) {
        ...
        // Show welcome message and close after 3 seconds
        setToast({ type: "success", message: `Welcome, ${data.user?.name || "User"}!` })
        setTimeout(() => {
          onClose()
        }, 3000)
      } else {
