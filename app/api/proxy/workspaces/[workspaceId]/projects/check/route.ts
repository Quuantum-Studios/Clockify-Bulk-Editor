...
export async function POST(req: NextRequest, context: { params: { workspaceId: string } }) {
  try {
    ...
    const { apiKey, projectNames = [] as string[] } = body
    ...
    const existing = allProjects.filter((p) => projectNames.map((n: string) => n.toLowerCase().trim()).includes(p.name.toLowerCase().trim()))
